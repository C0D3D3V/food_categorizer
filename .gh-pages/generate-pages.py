...
json_path = f"debug_{default_dir_paths.generated_vegattributes_json}"
            ...
            with list_md_path.open("w") as list_f:
                for food in sorted(foods, key=lambda x: x["description"]):
                    ...
                    comma_sep_discrepancies = ", ".join(
                        ...
                        for disc_source, disc_category in food.get("vegCategoryDiscrepancies", {}).items()
                    )
                    optional_discrepancies = (
                        f'; <span style="color: red;">{comma_sep_discrepancies}</span>'
                        if comma_sep_discrepancies
                        else ''
                    )
                    url = get_fdc_app_details_url(fdc_id)
                    list_f.write(
                        f"- {description} (fdcId: [{fdc_id}]({url}); " f"via {source}{optional_discrepancies})\n"
                    )

    data_dir = output_dir / "_data"
    data_dir.mkdir(exist_ok=True)
    with ReferenceSamplesCsv.from_path(default_dir_paths.reference_samples_csv) as reference_samples_csv, (
        data_dir / "stats.yml"
    ).open("w") as stats_file:
        reference_sample_dicts = list(reference_samples_csv.read_all_reference_sample_dicts())
        n_known_failures = sum(1 for r in reference_sample_dicts if r["known_failure"])
