...
class AutoIndexingJsonableWriter(Generic[T]):
    ...
    def put_entries(self, jsonables: Iterable[T]):
        self.jsonable_store.put_entries(self.primary_index.name, self._iter_primary_indexed_jsonables(jsonables))
        self._write_secondary_indices()

    def _iter_primary_indexed_jsonables(self, jsonables: Iterable[T]) -> Generator[Tuple[str, T], None, None]:
        for jsonable in jsonables:
            primary = self.primary_index.func(jsonable)
            for secondary in self.secondary_indices:
                self.secondary_maps[secondary.name][secondary.func(jsonable)].append(primary)
    ...
    def _write_secondary_indices(self):
        for name, map in self.secondary_maps.items():
            self.jsonable_store.put_links(
                name,
                ((k, [(self.primary_index.name, target) for target in targets]) for k, targets in map.items()),
            )
...
class IndexedJsonableStore(
    ...
    def __init__(
        ...
        self.auto_indexing_writer: AutoIndexingJsonableWriter[T] = AutoIndexingJsonableWriter(
