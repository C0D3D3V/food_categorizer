...
class ZippedIndexableLinkableBytesStore(
    ...
    def put_links(self, index_name: str, index_values_and_targets: Iterable[Tuple[str, LinkTargets]]):
        try:
    ...
    def _get_entry_no_resolve(self, index_name: str, index_value: str) -> bytes:
        try:
            ...
            raise KeyError(f"Entry for {index_name}={index_value} not in indexed JSON file") from e
    ...
    def _iter_data_index(self, index_name: str) -> Iterable[str]:
        return (Path(x).name for x in self.zipfile.namelist() if x.startswith(f"by-{index_name}/data/"))

    def _is_data_index(self, index_name: str) -> bool:
        return any(x.startswith(f"by-{index_name}/data/") for x in self.zipfile.namelist())

    def _is_link_index(self, index_name: str) -> bool:
        return any(x.startswith(f"by-{index_name}/links.json") for x in self.zipfile.namelist())
    ...
    def _resolve(self, index_name: str, index_value: str) -> Iterable[Tuple[str, str]]:
