...
from ..fooddata import load_sr_legacy_fooddata_dicts, load_survey_fooddata_dicts
...
def main():
    ...
    food_ds = load_survey_fooddata_dicts(default_dir_paths.survey_fooddata_json) + load_sr_legacy_fooddata_dicts(
        default_dir_paths.sr_legacy_fooddata_json
    )
    print("done")
    ...
    foods_in_categories = {category: [] for category in Category}
    fdc_categories_to_foods_in_veg_categories = defaultdict(lambda: {veg_category: [] for veg_category in Category})
    ...
    with default_food_and_reference_sample_stores() as (food_store, reference_sample_store):
        ...
        for i, food in enumerate(foods):
            ...
            fdc_categories_to_foods_in_veg_categories[food.fdc_category_description][categorization.category].append(
                food
            )
        print("done")
    ...
    for fdc_category, veg_categories_items in sorted(
        fdc_categories_to_foods_in_veg_categories.items(), key=lambda x: x[0]
    ):
    ...
    # export JSON
    for debug in ['debug_', '']:
        dict_from_food_func = vegattributes_dict_from_food if not debug else verbose_vegattributes_dict_from_food
