...
@pytest.fixture
def fake_survey_fooddata_json(tmp_path: Path, fake_food_data: List[FoodDataDict]):
...
def fake_fooddata_jsons(
    ...
    return FakeFoodDataJsons(survey=fake_survey_fooddata_json, sr_legacy=fake_sr_legacy_fooddata_json)
...
def fake_reference_sample_dicts(fake_food_data: List[FoodDataDict]):
    return [
        dict(fdc_id=food_data_dict["fdcId"], expected_category="VEGAN", known_failure=False, description=None)
...
def create_fake_reference_samples_csv(tmp_path):
    def _create_fake_reference_samples_csv(reference_sample_dicts: List[ReferenceSampleDict]) -> FakeReferenceSampleCsv:
...
def fake_reference_samples_csv_no_description(
    ...
    fake_reference_sample_dicts = [ChainMap({"description": None}, d) for d in fake_reference_sample_dicts]
    yield create_fake_reference_samples_csv(fake_reference_sample_dicts)
