...
from fooddata_vegattributes.utils.indexed_jsonable_store.zipped_indexable_linkable_jsonable_store import (
...
def test_write_and_read_entries(sample_entries_by_id: Dict[str, Any], tmp_path: Path, caching: bool):
    path = tmp_path / "store.zip"
    with ZippedIndexableLinkableJsonableStore.from_path(path, mode="w", caching=caching) as store:
    ...
    with ZippedIndexableLinkableJsonableStore.from_path(path, mode="r", caching=caching) as store:
        assert store.get_entry("id", "1") == [1, 2, 3]
        assert store.get_entry("id", "2") == "hello"
...
@pytest.mark.parametrize("caching", [True, False])
def test_links(sample_entries_by_id: Dict[str, Any], tmp_path: Path, caching: bool):
    path = tmp_path / "store.zip"
    with ZippedIndexableLinkableJsonableStore.from_path(path, mode="w", caching=caching) as store:
