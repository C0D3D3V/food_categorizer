...
from fooddata_vegattributes.utils.indexed_jsonable_store.caching_zipped_indexable_linkable_bytes_store import (
...
def test_write_and_read_entries(sample_entries_by_id: Dict[str, bytes], tmp_path: Path):
    path = tmp_path / "store.zip"
    with CachingZippedIndexableLinkableBytesStore.from_path(path, mode="w") as store:
    ...
    with CachingZippedIndexableLinkableBytesStore.from_path(path, mode="r") as store:
        assert store.get_entry("id", "1") == b"hello"
        assert store.get_entry("id", "2") == b"world"
        assert sorted(list(store.iter_index("id"))) == ["1", "2", "3"]


def test_links(sample_entries_by_id: Dict[str, bytes], tmp_path: Path):
    path = tmp_path / "store.zip"
    with CachingZippedIndexableLinkableBytesStore.from_path(path, mode="w") as store:
