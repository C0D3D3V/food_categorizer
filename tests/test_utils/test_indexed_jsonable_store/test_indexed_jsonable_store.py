...
def test_write_and_read_entries_and_links(sample_entries: List[Dict[str, Any]], tmp_path: Path, caching: bool):
    ...
    with IndexedJsonableStore.from_path(
        ...
        assert sorted(x["name"] for x in store.iter_entries("profession", "Accountant")) == ["Harold", "Olivia"]
        # test nonexistent indices
        with pytest.raises(KeyError, match="Entry for .* not in indexed JSON file"):
